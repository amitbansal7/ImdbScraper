...
object ReviewScraper {
  ...
  def getAllreviews(id: String, limit: Int, proxy: (String, Int)): List[Review] = {
    ...
    def findWithKey(str: String, id: String, count: Int, limit: Int): List[Review] = {
      if (str.equals("") || count >= limit) Nil
        ...
        val doc = JsoupDoc.getJsoupDoc("http://www.imdb.com/title/" + id + "/reviews/_ajax?ref_=undefined&paginationKey=" + str, proxy)
    ...
    val doc = JsoupDoc.getJsoupDoc("http://www.imdb.com/title/" + id + "/reviews/?sort=submissionDate&dir=desc", proxy)
